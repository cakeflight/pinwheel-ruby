...
module Pinwheel
  class JobObjResponseV20231122
    ...
    # On error, an explicit code describing the error.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Pinwheel::JobObjResponseV20231122` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Pinwheel::JobObjResponseV20231122`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      name_validator = EnumAttributeValidator.new('String', ["direct_deposit_switch", "direct_deposit_allocations", "shifts", "paystubs", "employment", "income", "tax_forms", "identity", "direct_deposit_payment"])
      return false unless name_validator.valid?(@name)
      return false if @timestamp.nil?
      return false if @outcome.nil?
      outcome_validator = EnumAttributeValidator.new('String', ["error", "pending", "success"])
    ...
    def name=(name)
      validator = EnumAttributeValidator.new('String', ["direct_deposit_switch", "direct_deposit_allocations", "shifts", "paystubs", "employment", "income", "tax_forms", "identity", "direct_deposit_payment"])
    ...
    def outcome=(outcome)
      validator = EnumAttributeValidator.new('String', ["error", "pending", "success"])
    ...
    def hash
      [id, name, timestamp, outcome, error_code, error_type, link_token_id, params, account_id].hash
    end
