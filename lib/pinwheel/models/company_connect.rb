...
module Pinwheel
  ...
  class CompanyConnect
    # ID of the Company Connection.
    attr_accessor :company_connection_id

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        company_connection_id: :company_connection_id
      }
    end
    ...
    def initialize(attributes = {})
      if !attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `Pinwheel::CompanyConnect` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if !self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `Pinwheel::CompanyConnect`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:company_connection_id)
        self.company_connection_id = attributes[:company_connection_id]
      end
    end
    ...
    def ==(other)
      ...
      self.class == other.class &&
        company_connection_id == other.company_connection_id
    end
