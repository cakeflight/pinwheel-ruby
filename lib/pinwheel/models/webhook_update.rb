  ...
  class WebhookUpdate
    ...
    def initialize(attributes = {})
      if !attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `Pinwheel::WebhookUpdate` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if !self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `Pinwheel::WebhookUpdate`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      invalid_properties = []
      if !@enabled_events.nil? && @enabled_events.length < 1
    ...
    def valid?
      ...
      return false if !@enabled_events.nil? && @enabled_events.length < 1
      true
    end
    ...
    def ==(other)
      ...
      self.class == other.class &&
        url == other.url &&
        status == other.status &&
        enabled_events == other.enabled_events
    end
    ...
    def hash
      [url, status, enabled_events].hash
    end
