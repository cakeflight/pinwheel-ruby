...
module Pinwheel
  class CompanyPaystubObjResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Pinwheel::CompanyPaystubObjResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Pinwheel::CompanyPaystubObjResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [created_at, employee_external_id, pay_date, pay_period_start, pay_period_end, currency, gross_pay_amount, net_pay_amount, check_amount, gross_pay_ytd, net_pay_ytd, total_taxes, total_deductions, total_reimbursements, employer_name].hash
    end
