...
module Pinwheel
  class BankAccountDetails
    ...
    # Routing number of bank. Required if requesting direct_deposit_switch jobs.
    attr_accessor :routing_number

    # Bank account number. Required if requesting direct_deposit_switch jobs.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Pinwheel::BankAccountDetails` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Pinwheel::BankAccountDetails`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if !@routing_number.nil? && @routing_number.to_s.length < 9
      return false if !@routing_number.nil? && @routing_number !~ Regexp.new(/^[0-9]*$/)
      return false if !@account_number.nil? && @account_number.to_s.length < 6
      return false if !@account_number.nil? && @account_number !~ Regexp.new(/^[0-9]*$/)
      true
    end
    ...
    def hash
      [name, type, routing_number, account_number].hash
    end
