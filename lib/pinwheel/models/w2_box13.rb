...
module Pinwheel
  class W2Box13
    # Statutory employee option. Box 13 IRS Form W-2.
    attr_accessor :statutory_employee

    # Retirement plan option. Box 13 IRS Form W-2.
    attr_accessor :retirement_plan

    # Third-party sick pay option. Box 13 IRS Form W-2.
    ...
    def initialize(attributes = {})
      if !attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `Pinwheel::W2Box13` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if !self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `Pinwheel::W2Box13`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def ==(other)
      ...
      self.class == other.class &&
        statutory_employee == other.statutory_employee &&
        retirement_plan == other.retirement_plan &&
        third_party_sick_pay == other.third_party_sick_pay
    end
    ...
    def hash
      [statutory_employee, retirement_plan, third_party_sick_pay].hash
    end
