...
module Pinwheel
  class EarningsStreamPayoutObjResponseV20230418
    attr_accessor :status

    # Date that the payout will arrive in a user's account. This date includes a buffer for ACH settlement time. For estimated payouts, this is projected from historical pay dates and may differ by a few days from actual settlement. For processed payouts, this is taken directly from the paystub.
    attr_accessor :pay_date

    # Object representing how much someone will receive in net pay (i.e., the pay that will be disbursed to user-directed accounts) for the work they completed that day. This is equivalent to (gross pay - deductions). E.g., gross pay minus income tax, healthcare, 401k contributions, or other withholdings that are not included in the user's payout.
    attr_accessor :net_pay

    # List of objects representing income earned on a given day. All earnings roll up into specific payouts.
    ...
    def initialize(attributes = {})
      if !attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `Pinwheel::EarningsStreamPayoutObjResponseV20230418` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if !self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `Pinwheel::EarningsStreamPayoutObjResponseV20230418`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def status=(status)
      validator = EnumAttributeValidator.new("String", ["estimated", "processed"])
    ...
    def hash
      [status, pay_date, net_pay, earnings].hash
    end
